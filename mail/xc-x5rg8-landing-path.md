...
- the remaining landing work is the follow-on `xenon#26` CLI warm-up fix plus
  top-level human review/manual-QA on the refreshed packet and final
  parent-bead closure
...
- one intentional `xenon` submodule pointer change for `xenon#26`
- no placeholder `.beads` payload
- no top-level code delta that tries to re-land the already-landed cortex
  lifecycle work
- current landing packet artifacts under `mail/`
