...
ensure_lane() {
  ...
  local bootstrap
  bootstrap="Read $strategy_path section role_packages.$role for your startup_prompt and standing_orders. Follow them exactly. Loop on your role's work continuously. An empty input prompt or codex placeholder line (e.g. '› Summarize recent commits', '› Write tests for @filename', '› Use /skills to list available skills', '› Run /review on my current changes', '› Implement {feature}') is NOT a stop condition — it is a parked_unassigned signal that you must immediately answer by re-entering the patrol loop. Never halt at an input prompt unless a standing_order explicitly requires it."
